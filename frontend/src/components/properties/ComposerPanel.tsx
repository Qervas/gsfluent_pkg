 ...
 *
 *  On any pick change we call POST /api/compose and load the result as the new
 *  recipe baseline (loadActiveRecipe — sets simRecipeBaseline + clears
 *  overrides in one shot). An over-ceiling / unknown pick returns a 422 whose
 *  reason we surface inline rather than silently doing nothing. */
export function ComposerPanel() {
  ...
  const onScenario = (sc: string) => {
    ...
    const rec = lib.scenarios.find((x) => x.name === sc)?.recommended_material;
    const mat = rec ?? m;
    setMaterial(mat);
