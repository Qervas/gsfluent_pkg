...
Run scripts/install-local.sh first; it creates the venv + builds the SPA.
EOF
    exit 1
fi
...
Run scripts/install-local.sh first — it builds the SPA into frontend/dist/.
