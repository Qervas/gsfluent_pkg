#!/usr/bin/env bash
# Laptop-local installer (implementation detail — call via npm).
#
# The teammate-facing entry point is `npm install` from frontend/, which
# fires this script as a postinstall hook. This file stays callable
...
#   3. Runs `npm run build` so frontend/dist/ is ready
#      (npm install of frontend/node_modules happens BEFORE this script
#      when invoked via postinstall; set GSFLUENT_SKIP_NPM_INSTALL=1 to
#      skip the `npm ci` step here. When called directly, this script
#      runs `npm ci` itself.)
#
...
#
# Usage:
#   npm --prefix frontend install         # normal teammate path
#   bash scripts/_install.sh              # direct (for debugging)
#   bash scripts/install-local.sh         # legacy shim, still works
#
# Env:
#   PYTHON_BIN                  python3.10+ to use (default: python3)
#   PYTHON_ONLY                 if 1, skip the SPA build entirely
...
if [[ "${PYTHON_ONLY:-0}" == "1" ]]; then
    ...
    if [[ "${GSFLUENT_SKIP_NPM_INSTALL:-0}" == "1" ]]; then
        note "GSFLUENT_SKIP_NPM_INSTALL=1 — skipping npm ci (already installed by parent)"
    else
...
echo ""
echo "Next:"
echo "  cd frontend && npm start         # (recommended)"
echo "  bash scripts/_start.sh           # (direct)"
