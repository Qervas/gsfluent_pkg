...
To author a sim, use the **[Compose](#compose-material--scenario--building)**
endpoints (material × scenario × building → flat recipe), then submit the
result to [`POST /api/runs`](#post-apiruns). The typical flow:
...
## Compose (material × scenario × building)

The primary way to author a sim. Instead of hand-editing a flat recipe, you
pick three orthogonal inputs — **MATERIAL × SCENARIO × BUILDING** — and the
backend composes the flat recipe that `POST /api/runs` consumes. Source of
truth: `server/gsfluent/authoring/` (`materials.py`, `scenarios.py`,
`buildings.py`, `compose.py`). The composer is pure + deterministic — no
GPU, no sim. Both endpoints read the authoring modules live, so new
scenarios/materials appear with no API change.

### GET /api/compose/library

List the three libraries for the picker dropdowns.
...
numerically eject *stiff* materials (jelly/plasticine) with a grid-escape
...
Compose a flat recipe from the three picks.
...
| Field | Type | Description |
| --- | --- | --- |
| `material` | string | A material name from `GET /api/compose/library`. |
| `scenario` | string | A scenario name. |
| `building` | string | A building name. |
...
```json
{
  ...
  "recipe_data": { "...": "the flat recipe — forward this to POST /api/runs" }
}
```

`recipe_data` is the exact object you pass back as `recipe_data` to
`POST /api/runs`. It carries a `_composed_from` provenance block
...
before using them as a `run_name`.

**Status codes**

| Code | Cause |
| --- | --- |
| 422 | Unknown material/scenario/building (the message lists valid names), or an over-ceiling value (e.g. an impact speed past the grid-escape limit). Nothing is silently clamped — the `error.message` says why. |
...
> **Composer vs saved recipes.** The five destruction scenarios above are
> *composed* (in-memory), not listed here. The saved recipes are the flat
> material demos (jelly/metal/sand/foam/plasticine) + the `demolition`
> fallback + any `★` user presets.
