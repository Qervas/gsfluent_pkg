...
### Task 2.5: Deploy the Phase-2 server changes to sxyin-host
...
```bash
cd /home/frankyin/Desktop/work/gsfluent_pkg
rsync -avz server/gsfluent/api/models.py sxyin-host:/data/yinshaoxuan/gsfluent_pkg_tmp/server/gsfluent/api/models.py
rsync -avz server/gsfluent/core/models.py sxyin-host:/data/yinshaoxuan/gsfluent_pkg_tmp/server/gsfluent/core/models.py
```

- [ ] **Step 2: Remove the now-unused server-side helper**

```bash
ssh sxyin-host 'rm -f /data/yinshaoxuan/gsfluent_pkg_tmp/server/gsfluent/core/static_to_viser.py'
```
...
nohup env GSFLUENT_SIM_PYTHON=/data/yinshaoxuan/miniconda3/envs/GaussianFluent/bin/python GSFLUENT_SIM_HOME=/data/yinshaoxuan/GaussianFluent /data/yinshaoxuan/miniconda3/envs/gsfluent-api/bin/gsfluent serve --host 0.0.0.0 --port 18080 --no-browser > /data/yinshaoxuan/gsfluent_pkg_tmp/work/logs/server.log 2>&1 & disown
sleep 3
...
- [ ] Switch to Splat mode for the freshly-uploaded model — viser loads the .ply directly (verify by `tail /data/yinshaoxuan/gsfluent_pkg_tmp/work/logs/server.log` or viser's stderr showing the model-fetch line). May take a few seconds the first time.
...
```bash
ssh sxyin-host 'ls /data/yinshaoxuan/gsfluent_pkg_tmp/tools/local_stream.py 2>/dev/null'
# If it's there, remove the SERVER copy too:
ssh sxyin-host 'rm -f /data/yinshaoxuan/gsfluent_pkg_tmp/tools/local_stream.py'
...
ssh sxyin-host 'curl -s http://localhost:18080/api/runs'
# Expect: []
