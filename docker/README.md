...
```
  laptop                                server (your-server)
                                        ┌──────────────────────────┐
                                        │ Docker container         │
                                        │  - gsfluent server       │
  ssh -L 8080:localhost:8080 ──────────►│  - GPU access (--gpus all)│
  your-server                            │  - mounts host paths for │
                                        │    library + cache       │
...
Example: run on port 18080 with the existing your-server library dir:

```bash
BACKEND_PORT=18080 \
LIBRARY_HOST_DIR=$USER_HOME/gsfluent_work/library \
CACHE_HOST_DIR=$USER_HOME/gsfluent_work/cache \
docker compose -f docker/compose.yml up -d
```
...
ssh -N -L 8080:localhost:8080 your-server
