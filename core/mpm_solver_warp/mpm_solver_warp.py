...
class MPM_Simulator_WARP:
    def __init__(self, n_particles, n_grid=100, grid_lim=1.0, device="cuda:0",
        ...
        # [PhaseB.2] mixed_precision = True puts particle x, v, C in fp16 sidecars.
        # Stress / F / F_trial / vol / mass stay fp32. Grid stays fp32.
        # Default False = byte-identical to pre-B.2 behavior.
        self.mixed_precision = bool(mixed_precision)
        self.particle_x_h = None
        self.particle_v_h = None
        self.particle_C_h = None
        self._fp16_initialized = False
        # [Phase C.2.a Pass 2] sort-by-cell P2G knob. Default False keeps the
        # off-path byte-identical to post-B.3 behavior. The runner flips this
        # via mpm_solver.sort_p2g = True before the warmup + capture sequence.
        self.sort_p2g = False
    ...
    def initialize(self, n_particles, n_grid=100, grid_lim=1.0, device="cuda:0"):
        ...
        # [PhaseB.2] reset fp16 sidecars on re-init; init_fp16_mirror() will rebuild.
        self.particle_x_h = None
        self.particle_v_h = None
        self.particle_C_h = None
        self._fp16_initialized = False
        ...
        # [Phase C.2.a Pass 1] sort-by-cell scratch buffer slots.
        # We DO NOT allocate the real buffers here -- back-to-back A/B benches
        # showed an ~5.5% slowdown of the off path when the full-sized cell_id /
        # perm / cell_offset buffers (~30 MB total at 969k particles, n_grid=150)
        # were eagerly allocated, even though no kernel reads them.  The slowdown
        # appears to come from VRAM-layout / allocator-pool effects on subsequent
        # kernels (the building scene runs concurrently with other GPU jobs that
        # share memory bus pressure).  To keep Pass 1 byte-for-byte free for the
        # off path we expose only the field placeholders + a one-call allocator
        # ensure_sort_buffers().  Pass 2 will call it once before wp.capture_begin
        # so the scratch is sized correctly and warp.utils.radix_sort_pairs has
        # its CUB temp pool warmed up before graph capture.
        self.cell_id = None
        self.perm = None
        self.cell_offset = None
        self._sort_buffers_ready = False
        self._sort_device = device  # remembered for ensure_sort_buffers
    ...
    # [Phase C.2.a Pass 1] lazy allocator for sort-by-cell scratch.  Idempotent
    # and re-init-safe: if n_particles or n_grid changed since last allocation,
    # the buffers are reallocated.  Pass 1 does NOT call this from anywhere --
    # this is scaffolding so Pass 2 can do `solver.ensure_sort_buffers()` once
    # before wp.capture_begin to pre-allocate scratch and warm up
    # warp.utils.radix_sort_pairs's internal CUB temp-storage cache.
    def ensure_sort_buffers(self, device=None):
        if device is None:
            device = self._sort_device
        ...
        # cell_id / perm sized 2*n_particles -- warp.utils.radix_sort_pairs in
        # Warp 0.10.1 asserts size >= 2*count for double-buffered keys/values.
        need_realloc = (
            self.cell_id is None
            or self.cell_id.size != 2 * n_particles
            or self.perm is None
            or self.perm.size != 2 * n_particles
            or self.cell_offset is None
            or self.cell_offset.size != n_cells + 1
        )
        if need_realloc:
            self.cell_id = wp.zeros(shape=(2 * n_particles,), dtype=wp.int32, device=device)
            self.perm = wp.zeros(shape=(2 * n_particles,), dtype=wp.int32, device=device)
            self.cell_offset = wp.zeros(shape=(n_cells + 1,), dtype=wp.int32, device=device)
            self._sort_device = device
        self._sort_buffers_ready = True
    ...
    def p2g2p_capture_safe(self, step, dt, device="cuda:0", flip_pic_ratio: float = 0.80, flip_pic: bool = True):
        """Phase A: capture-safe substep — no ScopedTimer (no host sync), suitable for wp.capture_begin/end.
        Phase B.2: when self.mixed_precision is True and init_fp16_mirror() has run,
        the p2g/g2p kernels read/write the fp16 sidecars instead of state.particle_{x,v,C}."""
        grid_size = (
            self.mpm_model.grid_dim_x,
            self.mpm_model.grid_dim_y,
            self.mpm_model.grid_dim_z,
        )
        use_fp16 = self.mixed_precision and self._fp16_initialized
        ...
        for k in range(len(self.pre_p2g_operations)):
            wp.launch(kernel=self.pre_p2g_operations[k], dim=self.n_particles,
                      inputs=[self.time, dt, self.mpm_state, self.impulse_params[k]], device=device)
        for k in range(len(self.particle_velocity_modifiers)):
            wp.launch(kernel=self.particle_velocity_modifiers[k], dim=self.n_particles,
                      inputs=[self.time, self.mpm_state, self.particle_velocity_modifier_params[k]], device=device)
        wp.launch(kernel=compute_stress_from_F_trial, dim=self.n_particles,
                  inputs=[self.mpm_state, self.mpm_model, dt], device=device)
        # [Phase C.2.a Pass 2] sorted-P2G path. Gated on self.sort_p2g; only
        # active for the fp32 + flip_pic branch (the dominant production case
        # at 969k particles). Other branches (fp16, APIC) fall through to the
        # original unsorted launches below for safety -- they are not on the
        # hot path being measured.
        sort_active = (self.sort_p2g and (not use_fp16) and flip_pic)
        if sort_active:
            cell_id = self.cell_id
            perm = self.perm
            wp.launch(kernel=compute_cell_id, dim=self.n_particles,
                      inputs=[self.mpm_state.particle_x, self.mpm_model.inv_dx,
                              self.mpm_model.n_grid, cell_id, perm], device=device)
            wp.utils.radix_sort_pairs(cell_id, perm, self.n_particles)
            wp.launch(kernel=p2g_flip_pic_sorted, dim=self.n_particles,
                      inputs=[self.mpm_state, self.mpm_model, dt, perm], device=device)
        elif use_fp16:
            if flip_pic:
                wp.launch(kernel=p2g_flip_pic_with_stress_fp16, dim=self.n_particles,
                          inputs=[self.mpm_state, self.mpm_model, dt,
                                  self.particle_x_h, self.particle_v_h], device=device)
            else:
                wp.launch(kernel=p2g_apic_with_stress_fp16, dim=self.n_particles,
                          inputs=[self.mpm_state, self.mpm_model, dt,
                                  self.particle_x_h, self.particle_v_h, self.particle_C_h], device=device)
        else:
            if flip_pic:
                wp.launch(kernel=p2g_flip_pic_with_stress, dim=self.n_particles,
                          inputs=[self.mpm_state, self.mpm_model, dt], device=device)
            else:
        ...
        if self.mpm_model.grid_v_damping_scale < 1.0:
            wp.launch(kernel=add_damping_via_grid, dim=grid_size,
                      inputs=[self.mpm_state, self.mpm_model.grid_v_damping_scale], device=device)
        for k in range(len(self.grid_postprocess)):
            wp.launch(kernel=self.grid_postprocess[k], dim=grid_size,
                      inputs=[self.time, dt, self.mpm_state, self.mpm_model, self.collider_params[k]], device=device)
            if self.modify_bc[k] is not None:
                self.modify_bc[k](self.time, dt, self.collider_params[k])
        if use_fp16:
            if flip_pic:
                wp.launch(kernel=g2p_flip_fp16, dim=self.n_particles,
                          inputs=[self.mpm_state, self.mpm_model, dt, flip_pic_ratio,
                                  self.particle_x_h, self.particle_v_h], device=device)
            else:
                wp.launch(kernel=g2p_fp16, dim=self.n_particles,
                          inputs=[self.mpm_state, self.mpm_model, dt,
                                  self.particle_x_h, self.particle_v_h, self.particle_C_h], device=device)
        else:
            if flip_pic:
                wp.launch(kernel=g2p_flip, dim=self.n_particles,
                          inputs=[self.mpm_state, self.mpm_model, dt, flip_pic_ratio], device=device)
            else:
    ...
    def p2g2p(self, step, dt, device="cuda:0" , flip_pic_ratio: float = 0.80, flip_pic: bool =True):
        ...
        # p2g
        # [PhaseB.2] dispatch fp16 variant when mixed precision is on.
        _use_fp16 = self.mixed_precision and self._fp16_initialized
        # [Phase C.2.a Pass 2] sorted-P2G path (flag self.sort_p2g, fp32 + flip_pic only)
        _sort_active = (self.sort_p2g and (not _use_fp16) and flip_pic)
        with wp.ScopedTimer(
            "p2g",
            synchronize=True,
            print=False,
            ...
            if _sort_active:
                wp.launch(
                    kernel=compute_cell_id, dim=self.n_particles,
                    inputs=[self.mpm_state.particle_x, self.mpm_model.inv_dx,
                            self.mpm_model.n_grid, self.cell_id, self.perm],
                    device=device,
                )
                wp.utils.radix_sort_pairs(self.cell_id, self.perm, self.n_particles)
                wp.launch(
                    kernel=p2g_flip_pic_sorted, dim=self.n_particles,
                    inputs=[self.mpm_state, self.mpm_model, dt, self.perm],
                    device=device,
                )
            elif _use_fp16:
                if flip_pic:
                    wp.launch(
                        kernel=p2g_flip_pic_with_stress_fp16,
                        dim=self.n_particles,
                        inputs=[self.mpm_state, self.mpm_model, dt,
                                self.particle_x_h, self.particle_v_h],
                        device=device,
                    )
                else:
                    wp.launch(
                        kernel=p2g_apic_with_stress_fp16,
                        dim=self.n_particles,
                        inputs=[self.mpm_state, self.mpm_model, dt,
                                self.particle_x_h, self.particle_v_h, self.particle_C_h],
                        device=device,
                    )
            else:
                if flip_pic:
    ...
    def import_particle_x_from_torch(self, tensor_x, clone=True, device="cuda:0"):
        if tensor_x is not None:
            ...
            wp.copy(self.mpm_state.particle_x, torch2warp_vec3(tensor_x, dvc=device))
    ...
    def import_particle_v_from_torch(self, tensor_v, clone=True, device="cuda:0"):
        if tensor_v is not None:
            ...
            wp.copy(self.mpm_state.particle_v, torch2warp_vec3(tensor_v, dvc=device))
    ...
    def import_particle_F_from_torch(self, tensor_F, clone=True, device="cuda:0"):
        if tensor_F is not None:
            ...
            wp.copy(self.mpm_state.particle_F, torch2warp_mat33(tensor_F, dvc=device))
    ...
    def import_particle_C_from_torch(self, tensor_C, clone=True, device="cuda:0"):
        if tensor_C is not None:
            ...
            wp.copy(self.mpm_state.particle_C, torch2warp_mat33(tensor_C, dvc=device))
    ...
    # [PhaseB.2] fp16 sidecar lifecycle.
    def init_fp16_mirror(self, device="cuda:0"):
        """Allocate fp16 sidecars and copy current fp32 x/v/C into them.
        Call once after all particle state is set up (post BC setup, post
        impulse/initial-velocity import) and before the substep loop."""
        if not self.mixed_precision:
            return
        n = self.n_particles
        if self.particle_x_h is None or self.particle_x_h.shape[0] != n:
            self.particle_x_h = wp.zeros(shape=n, dtype=wp.vec3h, device=device)
            self.particle_v_h = wp.zeros(shape=n, dtype=wp.vec3h, device=device)
            self.particle_C_h = wp.zeros(shape=n, dtype=wp.mat33h, device=device)
        wp.launch(
            kernel=fp16_sync_from_fp32,
            dim=n,
            inputs=[self.mpm_state, self.particle_x_h, self.particle_v_h, self.particle_C_h],
            device=device,
        )
        self._fp16_initialized = True

    def sync_fp32_from_fp16(self, device="cuda:0"):
        """Pull fp16 sidecars back into the fp32 struct so external readers
        (export_particle_x_to_torch, BC kernels touching particle_x) see
        consistent state. Cheap (one kernel launch, ~36B per particle)."""
        if not self.mixed_precision or not self._fp16_initialized:
            return
        wp.launch(
            kernel=fp16_sync_to_fp32,
            dim=self.n_particles,
            inputs=[self.mpm_state, self.particle_x_h, self.particle_v_h, self.particle_C_h],
            device=device,
        )
