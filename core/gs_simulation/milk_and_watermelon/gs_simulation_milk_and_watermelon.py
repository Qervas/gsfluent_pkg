...
if __name__ == "__main__":
    ...
    mpm_init_pos[:, 2] += 0.2
    gaussians3 = load_checkpoint("$GSFLUENT_SIM_HOME/model/garden")
    ...
    # mpm_init_pos,  mpm_init_cov, mpm_init_vol,   opacity_render, shs_render = append_gaussian_data_flexible(
    #     "$GSFLUENT_SIM_HOME/model/dragonfruit",
    #     original_mean_pos,
    #     scale_origin,
    #     mpm_init_pos,
    #     mpm_init_cov,
    ...
    # gaussians2 = load_checkpoint("$GSFLUENT_SIM_HOME/model/garden_ours")
    ...
    azimuth_list , elvation_list, radius_list, center_list = generate_and_append_ellipse_path(start_azimuth=265, azimuth_max_delta= 20, start_elevation=10, path_radius=7.5, path_center=viewpoint_center_worldspace, elevation_max_delta=10, stage_num=15)
    gaussians_watermenlon = load_checkpoint("$GSFLUENT_SIM_HOME/model/watermelon")
    ...
    for frame in tqdm(range(frame_num)):
        ...
        if args.render_img:
            ...
            if color_flag:
                if  light_flag : 
                    np.save("$GSFLUENT_SIM_HOME/watermelon_frame/frame_20/pos.npy", pos.detach().cpu().numpy())
                    # np.save("$GSFLUENT_SIM_HOME/watermelon_frame/frame_20/pos.npy", pos.detach().cpu().numpy())
                    command = 'cd $GSFLUENT_SIM_HOME/ && source $(conda info --base)/etc/profile.d/conda.sh && conda activate GaussianFluent && python normal_vector_proc_nan.py'
                    run_command_realtime(command)


                    command = 'cd $GSFLUENT_SIM_HOME/ && source $(conda info --base)/etc/profile.d/conda.sh && conda activate GaussianFluent && python phong_model_wm_shs_15.py'
                    run_command_realtime(command)


                pos = torch.from_numpy(np.load("$GSFLUENT_SIM_HOME/watermelon_frame/frame_20/pos.npy")).to("cuda")
                valid_indice = torch.from_numpy(np.load("$GSFLUENT_SIM_HOME/watermelon_frame/frame_20/pos_valid_indice.npy")).to("cuda")
                colors = torch.from_numpy(np.load("$GSFLUENT_SIM_HOME/phong_colors.npy")).to("cuda").reshape(-1 , 3).float()
