...
_ti_arch_env = os.environ.get("GSFLUENT_TI_ARCH", "cuda").lower()
_ti_arch = ti.cpu if _ti_arch_env == "cpu" else ti.cuda
ti.init(arch=_ti_arch, device_memory_GB=2.0, random_seed=42)
print(f"[laptop] Taichi arch = {_ti_arch_env} (set GSFLUENT_TI_ARCH=cpu to force CPU for filling)", flush=True)
...
if __name__ == "__main__":
    ...
    if "impact" in raw_config:
        ...
        # diag
        _vt = mpm_solver.export_particle_v_to_torch()
        print(f"[diag] post-import v shape={_vt.shape} "
              f"mean=({_vt[:,0].mean():.3f},{_vt[:,1].mean():.3f},{_vt[:,2].mean():.3f}) "
              f"hot_mean=({_vt[_mask,0].mean():.3f},{_vt[_mask,1].mean():.3f},{_vt[_mask,2].mean():.3f})")
    else:
    ...
    def _b3_write_ply(filename, position_np):
    ...
    for frame in tqdm(range(frame_num)):
        ...
        if args.output_ply or args.output_h5:
            if args.async_io and args.output_ply and not args.output_h5:
                ...
                _io_futures.append(_io_executor.submit(_b3_write_ply, _ply_filename, _pos_host))
            else:
                save_data_at_frame(
                    mpm_solver,
                    directory_to_save,
