...
def validate_model_orientation(recipe_data: dict, model_dir: Path) -> None:
    """Compatibility hook for pre-run model orientation checks.

    This used to reject a submitted model when its longest bbox axis differed
    from the recipe building's longest axis. That worked for the original
    high-rise scan, but it was not a real "up" detector: valid buildings and
    object scans can have arbitrary dominant axes. Orientation is now an
    explicit user decision via ``POST /api/models/{name}/reorient`` and the
    viewport rotation controls. Keep this function as a no-op so existing API
    call sites remain stable while runs no longer block on a shape heuristic.
    """
    del recipe_data, model_dir
    return
