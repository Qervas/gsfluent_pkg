            ...
            if time >= param.start_time and time < param.end_time:
                ...
                if dotproduct < 0.0:
                    if param.surface_type == 0:
                        ...
                        # The original code overwrote v with vec3(0,0,0) here,
                        # forcing slip(1)/separate(2) to behave as full-stick
                        # (only sticky(0), handled in its own branch above,
                        # should zero the node). slip keeps the tangential
                        # component; separate keeps tangential + outward normal.
